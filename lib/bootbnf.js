...
import {def, confine} from './sesshim.js';
import {indent} from './indent.js';
import * as sc from './scanner.js';

export const {bnf, doBnf, metaCompile, quasifyParser, defaultBaseGrammar} = (function () {
  ...
  function compile(sexp) {
    ...
    const vtable = Object.freeze({
      bnf(...rules) {
        ...
        const tokenTypeListSrc = `[${[...tokenTypes].map(tt => JSON.stringify(tt)).join(', ')}]`;
...
if (value.length === 0) value = FAIL;`;
      ...
      super(ident) {
        return `[pos, value] = this.run(super.rule_${ident}, pos, ${JSON.stringify(ident)});`;
      },
      apply(hole) {
        numSubs = Math.max(numSubs, hole + 1);
  ...
  function metaCompile(baseRules, _ = void 0) {
    ...
    return function (...baseActions) {
      ...
      function _extends(baseQuasiParser) {
        return (template, ...subs) => quasiParser(template, ...subs)._asExtending(baseQuasiParser);
      }
  ...
  const bnfRules = [
    ...
    ['def', 'choice', ['or', ['act', ['seq', 'HOLE'], 3], ['act', ['seq'], 4]]],
    ['def', 'seq', ['*', 'term']],
    [
      'def',
      'term',
