...
export const {
  ...
  class Token {
    ...
    static tokensInSegment(segmentNum, segment, RE) {
      ...
      while (RE.lastIndex < segment.length) {
        ...
        if (arr === null) {
          const badTok = new Token(segment.slice(RE.lastIndex), new Pos(segmentNum, RE.lastIndex, segment.length));
          throw new SyntaxError(`Unexpected: ${badTok}`);
        }
        const text = arr[1];
        const actualStart = RE.lastIndex - text.length;
        const tok = new Token(text, new Pos(segmentNum, actualStart, RE.lastIndex));
  ...
  const TOKEN_RE = captureRE(anyRE(SPACE_RE, NUMBER_RE, STRING_RE, IDENT_RE, SINGLE_OP, MULTI_OP, LINE_COMMENT_RE));
    ...
    lastFailures() {
      ...
      if (this.constructor._debug) {
        ...
        for (let [pos, posm] of this._memo) {
          const fails = [];
          for (let [ruleOrPatt, result] of posm) {
            const name = typeof ruleOrPatt === 'function' ? ruleOrPatt.name : JSON.stringify(ruleOrPatt);
            if (result === LEFT_RECUR) {
        ...
        console.log(`hits: ${this._counts.hits}, misses: ${this._counts.misses}`);
      }
    }
  }
...
-------`);
      ...
      const tokStr =
        last < this.toks.length ? `At ${this.toks[last]}` : `Unexpected EOF after ${this.toks[this.toks.length - 1]}`;
      const failStr = fails.length === 0 ? `stuck` : `looking for ${fails.join(' ')}`;
    ...
    eat(pos, patt) {
      ...
      if (pos < this.toks.length) {
        const token = this.toks[pos];
        if (typeof token !== 'number') {
          if ((typeof patt === 'string' && patt === token.text) || allRE(patt).test(token.text)) {
    ...
    rule_NUMBER(pos) {
      return this.eat(pos, NUMBER_RE);
    }
    rule_STRING(pos) {
      return this.eat(pos, STRING_RE);
    }
    rule_IDENT(pos) {
      [pos] = this.rule_SKIP(pos);
      if (pos >= this.toks.length) {
      ...
      if (allRE(IDENT_RE).test(token.text) && !this.keywords.has(token.text)) {
        return [pos + 1, token.text];
      }
      return [pos, FAIL];
    }
