...
import {FAIL, EOF, SPACE_RE, NUMBER_RE, STRING_RE, IDENT_RE, LINE_COMMENT_RE, stickyRE, Packratter} from './scanner.js';
import {quasifyParser, bnf} from './bootbnf.js';

export const {BaseScannerless, baseScannerless, match, skip, scannerless} = (function () {
  /**
  ...
  function skip(RE) {
    ...
    return function skipper(pos) {
      ...
      if (Array.isArray(found)) {
        ...
        if (arr) {
          let value = arr.length === 1 ? arr[0] : arr.length === 2 ? arr[1] : arr.slice(1);
