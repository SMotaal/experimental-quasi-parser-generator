...
import {tinyses} from './tinyses.js';

export const {desugar, scope, interp} = (function () {
  ...
  class ScopeVisitor extends ReplaceVisitor {
    ...
    arrow(_, params, block) {
      return this.shadow(() => ['arrow', this.visitAsts(params), this.visitAst(block)]);
    }
  }
        ...
        params.forEach((param, i) => {
          const paramVisitor = def({
            ...
            optional(_, patt, expr) {
              const val = i < specimen.length ? specimen[i] : interp(expr, patternVisitor.env);
        ...
        propParams.forEach(propParam => {
          const propParamVisitor = def({
            ...
            optionalProp(_, key, patt, expr) {
              const k = interpKey(key, patternVisitor.env);
              const val = k in specimen ? specimen[k] : interp(expr, patternVisitor.env);
  ...
  class InterpVisitor {
    ...
    delete(_, fe) {
      return visit(fe, {
        get(_, base, index) {
          return delete this.i(base)[this.i(index)];
        },
      });
    }
    void(_, e) {
      return void this.i(e);
    }
    typeof(_, e) {
      // weird typeof scope
    ...
    '*='(_, lv, rv) {
      return this.assign(lv, o => o * this.i(rv));
    }
    '/='(_, lv, rv) {
      return this.assign(lv, o => o / this.i(rv));
    }
